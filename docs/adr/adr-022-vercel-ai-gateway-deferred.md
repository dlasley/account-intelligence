...
The reasoning: every LLM call in this system today happens server-side — narrative generation inside the backend worker service, and the cross-vendor audit as a standalone CLI run out-of-band. Both sit next to the data context those calls depend on (account history, prior signals, workspace configuration). The frontend has no direct path to that context, so any future frontend-facing AI feature would still have to go through the worker — meaning "frontend AI feature," in this architecture, actually means *frontend triggers the worker, the worker makes the call*. A gateway placed in front of the frontend would have nothing to route, because the frontend isn't the thing making LLM calls.
